...
void OutsetVerbEngine::prepare(const juce::dsp::ProcessSpec& spec)
{
    ...
    // Sync EQ node to saved APVTS values once on prepare (e.g. after preset load)
...
void OutsetVerbEngine::processBlock(juce::AudioBuffer<float>& buffer)
{
    ...
    // Update parameters from APVTS every block (EQ excluded — owned by drag editor)
    updateChainParameters();
...
void OutsetVerbEngine::updateChainParameters()
{
    ...
    // EQ parameters intentionally omitted here — the drag editor (ThreeBandEQEditor)
    // owns the EQ node directly and writes back to APVTS via writeToApvts().
    // They are only synced from APVTS in prepare() so preset state is restored correctly.
